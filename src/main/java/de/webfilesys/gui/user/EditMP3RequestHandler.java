...
public class EditMP3RequestHandler extends UserRequestHandler
{
	...
	protected void editMP3Form()
	{
		...
		{
		    String imgUrl = "/doxee-internal/servlet?command=mp3Thumb&path=" + UTF8URLEncoder.encode(path);
	        output.println("<tr><td class=\"formParm1\">");
        ...
        if (mobile != null) 
        {
            output.println("<input type=\"button\" value=\"" + getResource("button.close","Cancel") + "\" onclick=\"window.location.href='/doxee-internal/servlet?command=listFiles';\">");
        }
        else
        {
