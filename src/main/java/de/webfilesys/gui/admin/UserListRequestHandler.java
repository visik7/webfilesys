...
public class UserListRequestHandler extends AdminRequestHandler
{
	...
	protected void process()
	{
		...
		output.println("    {window.location=\"/doxee-internal/servlet?command=admin&cmd=deleteUser&userToBeDeleted=\" + delUser ;");
		output.println("    }");
		output.println("}");

		output.println("function diskQuota(userid) {window.open('/doxee-internal/servlet?command=diskQuota&userid=' + encodeURIComponent(userid) + '&random=' + new Date().getTime(),'quotaWin','scrollbars=no,resizable=no,width=400,height=230,left=100,top=100,screenX=100,screenY=100');}");
		...
		output.println("<form accept-charset=\"utf-8\" method=\"get\" action=\"/doxee-internal/servlet\">");
		...
		for (int i=0;i<usersOnPage.size();i++)
		{
			...
			output.print("<a href=\"/doxee-internal/servlet?command=admin&cmd=editUser&username=" + UTF8URLEncoder.encode(actUser.getUserid()) + "\"><img src=\"images/edit2.gif\" alt=\"Edit User\" border=0></a>");
		...
		if (paging.getElementNumber()>0)
		{
			...
			{
				output.print("<a href=\"/doxee-internal/servlet?command=admin&cmd=userList&startIndex=0\">");
				output.println("<img src=\"images/first.gif\" border=\"0\"></a>");

				output.println("&nbsp;");

				output.print("<a href=\"/doxee-internal/servlet?command=admin&cmd=userList&startIndex=" + paging.getPrevStartIndex() + "\">");
			...
			for  (int k = 0; k < startIndices.size(); k++)
			{
				...
				if (idx != paging.getStartIndex() - 1)
				{
					...
					{
						output.print("<a class=\"fn\" href=\"/doxee-internal/servlet?command=admin&cmd=userList&startIndex=" + idx + "\">");
			...
			{
				output.print("<a href=\"/doxee-internal/servlet?command=admin&cmd=userList&startIndex=" + paging.getNextStartIndex() + "\">");
				output.println("<img src=\"images/next.gif\" border=\"0\"></a>");

				output.println("&nbsp;");
				
				output.print("<a href=\"/doxee-internal/servlet?command=admin&cmd=userList&startIndex=" + paging.getLastPageStartIndex() + "\">");
		...
		output.println("<input type=\"button\" value=\"Add new user\" onclick=\"window.location.href='/doxee-internal/servlet?command=admin&cmd=registerUser'\">");
