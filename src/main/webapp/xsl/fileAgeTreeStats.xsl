...
<title><xsl:value-of select="/treeStats/resources/msg[@key='stats.ageWinTitle']/@value" /></title>
...
<xsl:template match="treeStats">
  ...
  <table class="dataForm" border="0" width="100%" cellspacing="0">
    ...
    <th class="datahead" style="border-right:1px solid #808080">
      <xsl:value-of select="/treeStats/resources/msg[@key='stats.fileAge']/@value" />
    </th>
    ...
    <xsl:for-each select="ageStats/cluster">  
      <tr>
        <td class="dataNumber sepTop" style="border-right:1px solid #808080">
          <xsl:value-of select="fileAge" />
        </td>
